/*jshint sub:true*/ (function () {
    ...
    function $$(selector, callback) {
        ...
        for (var i = 0; i < elems.length; ++i) {
            if (callback && typeof callback == "function") callback.call(this, elems[i]);
        }
    }
    ...
    window.GoAccess = window.GoAccess || {
        ...
        reconnect: function (wsConn) {
            if (this.retries >= this.maxRetries) return window.clearTimeout(this.wsTimer);
            this.retries++;
            if (this.currDelay < this.maxDelay) this.currDelay *= 2;
        ...
        setWebSocket: function (wsConn) {
            ...
            socket.onclose = function (event) {
                ...
                this.wsTimer = setTimeout(() => {
                    this.reconnect(wsConn);
                }, this.currDelay);
            }.bind(this);
        },
        setWebSocket2: function (wsConn) {
            var host = null,
                pingId = null;
            host = wsConn.url ? wsConn.url : window.location.hostname ? window.location.hostname : "localhost";
            var str = /^(wss?:\/\/)?[^\/]+:[0-9]{1,5}\//.test(host + "/") ? host : String(host + ":" + wsConn.port);
    ...
    GoAccess.Util = {
        ...
        formatBytes: function (bytes, decimals, numOnly) {
            if (bytes == 0) return numOnly ? 0 : "0 Byte";
            var k = 1024;
            var dm = decimals + 1 || 2;
        ...
        utime2str: function (usec) {
            if (usec >= 864e8) return (usec / 864e8).toFixed(2) + " d";
            else if (usec >= 36e8) return (usec / 36e8).toFixed(2) + " h";
            else if (usec >= 6e7) return (usec / 6e7).toFixed(2) + " m";
            else if (usec >= 1e6) return (usec / 1e6).toFixed(2) + " s";
            else if (usec >= 1e3) return (usec / 1e3).toFixed(2) + " ms";
        ...
        fmtValue: function (value, dataType, decimals, shorten) {
            ...
            switch (dataType) {
                ...
                case "date":
                    ...
                    break;
                case "numeric":
                    if (this.isNumeric(value)) val = shorten ? this.shortNum(value) : (+value).toLocaleString();
                    break;
                case "bytes":
                ...
                case "percent":
                    ...
                    break;
                case "time":
                    if (this.isNumeric(value)) val = value.toLocaleString();
                    break;
                case "secs":
                    var t = new Date(null);
                    t.setSeconds(value);
        ...
        isPanelValid: function (panel) {
            var data = GoAccess.getPanelData(),
                ui = GoAccess.getPanelUI();
            return !ui.hasOwnProperty(panel) || !data.hasOwnProperty(panel) || !ui[panel].id;
        },
        getCount: function (item) {
            if (this.isObject(item) && "count" in item) return item.count;
            return item;
        },
        getPercent: function (item) {
            if (this.isObject(item) && "percent" in item) return this.fmtValue(item.percent, "percent");
            return null;
        },
        isObject: function (o) {
            return o === Object(o);
    ...
    GoAccess.OverallStats = {
        ...
        renderData: function (data, ui) {
            ...
            if (GoAccess.Util.isPanelHidden("general")) return false;
            $(".wrap-general").innerHTML = GoAccess.AppTpls.General.wrap.render(GoAccess.Util.merge(ui, { from: data.start_date, to: data.end_date }));
            for (var x in data) {
                if (!data.hasOwnProperty(x) || !ui.items.hasOwnProperty(x)) continue;
    ...
    GoAccess.Nav = {
        ...
        toggleTables: function () {
            ...
            Object.keys(ui).forEach(
                function (panel, idx) {
                    if (!GoAccess.Util.isPanelValid(panel) || GoAccess.Util.isPanelHidden(panel)) ui[panel]["table"] = !showTables;
                }.bind(this)
            );
        ...
        getItems: function () {
            var ui = GoAccess.getPanelUI(),
                menu = [];
            for (var panel in ui) {
                if (GoAccess.Util.isPanelValid(panel)) continue;
                menu.push({ current: window.location.hash.substr(1) == panel, head: ui[panel].head, key: panel, icon: this.getIcon(panel), hidden: GoAccess.Util.isPanelHidden(panel) });
            }
            return menu;
        },
        ...
        renderMenu: function (e) {
            $(".nav-list").innerHTML = GoAccess.AppTpls.Nav.menu.render({ nav: this.getItems(), overall_current: window.location.hash.substr(1) == "", overall_hidden: GoAccess.Util.isPanelHidden("general"), labels: GoAccess.i18n });
            requestAnimationFrame(function () {
                $("nav").classList.toggle("active");
            });
            this.events();
    ...
    GoAccess.Panels = {
        ...
        closeOpts: function (e) {
            ...
            if (e.relatedTarget && e.relatedTarget.getAttribute("data-toggle") !== "dropdown") e.relatedTarget.click();
        },
        setPlotSelection: function (ui, prefs) {
            ...
            ui[chartType] = true;
            for (var i = 0, len = ui.plot.length; i < len; ++i) if (ui.plot[i].className == metric) ui.plot[i]["selected"] = true;
        },
        setColSelection: function (items, prefs) {
            var columns = (prefs || {}).columns || {};
            for (var i = 0, len = items.length; i < len; ++i) if (items[i].key in columns && columns[items[i].key]["hide"]) items[i]["hide"] = true;
        },
        setChartSelection: function (ui, prefs) {
            ui["showChart"] = prefs && "chart" in prefs ? prefs.chart : true;
        },
        setOpts: function (panel) {
            var ui = JSON.parse(JSON.stringify(GoAccess.getPanelUI(panel))),
                prefs = GoAccess.getPrefs(panel);
        ...
        enablePrev: function (panel) {
            ...
            if ($pagination) $pagination.parentNode.classList.remove("disabled");
        },
        disablePrev: function (panel) {
            var $pagination = $("#panel-" + panel + " .pagination a.panel-prev");
            if ($pagination) $pagination.parentNode.classList.add("disabled");
        },
        ...
        hasSubItems: function (ui, data) {
            for (var i = 0, len = data.length; i < len; ++i) {
                if (!data[i].items) return (ui["hasSubItems"] = false);
        ...
        resetPanel: function (panel) {
            var ui = GoAccess.getPanelUI();
            var ele = $("#panel-" + panel);
            if (GoAccess.Util.isPanelValid(panel) || GoAccess.Util.isPanelHidden(panel)) return false;
    ...
    GoAccess.Charts = {
        iter: function (callback) {
            Object.keys(GoAccess.AppCharts).forEach(function (panel) {
                if (!GoAccess.Util.isWithinViewPort($("#panel-" + panel))) return;
                if (callback && typeof callback === "function") callback.call(this, GoAccess.AppCharts[panel], panel);
            });
        },
        getMetricKeys: function (panel, key) {
            return GoAccess.getPanelUI(panel)["items"].map(function (a) {
                return a[key];
            });
        },
        ...
        redrawChart: function (targ) {
            ...
            GoAccess.setPrefs();
            for (var x in plotUI) {
                if (!plotUI.hasOwnProperty(x) || plotUI[x].className != plot) continue;
        ...
        extractCount: function (item) {
            var o = {};
            for (var prop in item) o[prop] = GoAccess.Util.getCount(item[prop]);
            return o;
        },
        processChartData: function (data) {
            var out = [];
            for (var i = 0; i < data.length; ++i) out.push(this.extractCount(data[i]));
            return out;
        },
        ...
        getXKey: function (datum, key) {
            var arr = [];
            if (typeof key === "string") return datum[key];
            for (var prop in key) arr.push(datum[key[prop]]);
            return arr.join(" ");
        },
        getAreaSpline: function (panel, plotUI, data) {
            var dualYaxis = plotUI["d3"]["y1"];
            var chart = AreaChart(dualYaxis)
                .labels({ y0: plotUI["d3"]["y0"].label, y1: dualYaxis ? plotUI["d3"]["y1"].label : "" })
                .x(
                    function (d) {
                        if ((((plotUI || {}).d3 || {}).x || {}).key) return this.getXKey(d, plotUI["d3"]["x"]["key"]);
                        return d.data;
                    }.bind(this)
                )
                .y0(function (d) {
                ...
                .height(175)
                .format({ x: (this.findUIItem(panel, "data") || {}).dataType || null, y0: ((plotUI.d3 || {}).y0 || {}).format, y1: ((plotUI.d3 || {}).y1 || {}).format })
                .opts(plotUI);
            dualYaxis &&
                chart.y1(function (d) {
        ...
        renderChart: function (panel, chart, data) {
            d3.select("#chart-" + panel + ">.chart-tooltip-wrap").remove();
        ...
        addChart: function (panel, ui) {
            ...
            if (!(chart = this.getChart(panel, plotUI, data))) return;
            this.renderChart(panel, chart, data);
            GoAccess.AppCharts[panel] = chart;
        },
    ...
    GoAccess.Tables = {
        ...
        events: function () {
            ...
            $$(
                ...
                function (item) {
                    item.onclick = function (e) {
                        if (!window.getSelection().toString()) this.toggleRow(e.currentTarget);
                    }.bind(this);
                }.bind(this)
            );
            $$(
                ".row-expandable.clickable",
                function (item) {
                    item.onclick = function (e) {
        ...
        sortColumn: function (ele) {
            ...
            GoAccess.Util.setProp(GoAccess.AppState, panel + ".sort", { field: field, order: order });
        ...
        getDataByKey: function (panel, key) {
            ...
            for (var i = 0, n = data.length; i < n; ++i) {
                if (GoAccess.Util.hashCode(data[i].data) == key) return data[i];
            }
            return null;
        },
        getSubItemsData: function (panel) {
            var out = [],
                items = this.chartData[panel];
            for (var x in items) {
                if (!items.hasOwnProperty(x)) continue;
        ...
        removeChartData: function (panel, key) {
            if (GoAccess.Util.getProp(this.chartData, panel + "." + key)) delete this.chartData[panel][key];
            if (!this.chartData[panel] || Object.keys(this.chartData[panel]).length == 0) return GoAccess.getPanelData(panel).data;
            return this.getSubItemsData(panel);
        },
        ...
        toggleRow: function (ele) {
            ...
            this.renderTable(panel, this.getCurPage(panel));
            if (!plotUI.redrawOnExpand) return;
            if (!hide) {
        ...
        getMetaCell: function (ui, o, key) {
            ...
            return { className: className, value: val ? GoAccess.Util.fmtValue(val, vtype) : null, percent: perc, title: ui.meta, label: ui.metaLabel || null };
        },
        ...
        getMetaRows: function (panel, ui, key) {
            ...
            for (var i = 0; i < uiItems.length; ++i) {
                var item = uiItems[i];
                if (this.hideColumn(panel, item.key)) continue;
                cells.push(this.getMetaCell(item, data[item.key], key));
            }
            return [{ hasSubItems: ui.hasSubItems, cells: cells, key: key.substring(0, 3) }];
        },
        renderMetaRow: function (panel, metarows, className) {
            var table = $(".table-" + panel + " tr." + className);
            if (!table) return;
            table.innerHTML = GoAccess.AppTpls.Tables.meta.render({ row: metarows });
        },
        iterUIItems: function (panel, uiItems, dataItems, callback) {
            var out = [];
            for (var i = 0; i < uiItems.length; ++i) {
                var uiItem = uiItems[i];
                if (this.hideColumn(panel, uiItem.key)) continue;
        ...
        getObjectCell: function (panel, ui, value) {
            var className = ui.className || "";
            className += ui.dataType != "string" ? "text-right" : "";
            return { className: className, percent: GoAccess.Util.getPercent(value), value: GoAccess.Util.fmtValue(GoAccess.Util.getCount(value), ui.dataType) };
        },
        ...
        renderRows: function (rows, panel, ui, dataItems, subItem, parentId) {
            ...
            if (dataItems.length == 0 && ui.items.length) {
                rows.push({ cells: [{ className: "text-center", colspan: ui.items.length + 1, value: "No data on this panel." }] });
            }
            var cellcb = null;
            for (var i = 0; i < dataItems.length; ++i) {
                ...
                switch (typeof dataItem) {
                    case "string":
                        data = dataItem;
                        cellcb = function () {
                            return { colspan: ui.items.length, value: data };
                        };
                        break;
                    default:
                        data = dataItem.data;
        ...
        renderDataRows: function (panel, ui, dataItems, page) {
            ...
            table.innerHTML = GoAccess.AppTpls.Tables.data.render({ rows: rows });
        },
        ...
        renderTables: function (force) {
            var ui = GoAccess.getPanelUI();
            for (var panel in ui) {
                if (GoAccess.Util.isPanelValid(panel) || GoAccess.Util.isPanelHidden(panel) || !this.showTables()) continue;
                if (force || GoAccess.Util.isWithinViewPort($("#panel-" + panel))) this.renderFullTable(panel);
            }
        },
    ...
    GoAccess.App = {
        ...
        setTpls: function () {
            GoAccess.AppTpls = {
                Nav: { wrap: this.tpl($("#tpl-nav-wrap").innerHTML), menu: this.tpl($("#tpl-nav-menu").innerHTML), opts: this.tpl($("#tpl-nav-opts").innerHTML) },
                Panels: { wrap: this.tpl($("#tpl-panel").innerHTML), opts: this.tpl($("#tpl-panel-opts").innerHTML) },
                General: { wrap: this.tpl($("#tpl-general").innerHTML), items: this.tpl($("#tpl-general-items").innerHTML) },
                Tables: {
        ...
        sortField: function (o, field) {
            var f = o[field];
            if (GoAccess.Util.isObject(f) && f !== null) f = o[field].count;
            return f;
        },
        sortData: function (panel, field, order) {
            ...
            panelData.sort(
                function (a, b) {
                    ...
                    if (typeof a === "string" && typeof b === "string") return "asc" == order ? a.localeCompare(b) : b.localeCompare(a);
        ...
        verifySort: function () {
            ...
            for (var panel in ui) {
                if (GoAccess.Util.isPanelValid(panel) || GoAccess.Util.isPanelHidden(panel)) continue;
                var sort = GoAccess.Util.getProp(GoAccess.AppState, panel + ".sort");
                if (JSON.stringify(sort) === JSON.stringify(ui[panel].sort)) continue;
        ...
        renderData: function () {
            ...
            this.verifySort();
            GoAccess.OverallStats.initialize();
            if (!GoAccess.AppState.updated) return;
    ...
    document.addEventListener("visibilitychange", function () {
        if (document.visibilityState === "hidden") GoAccess.App.hasFocus = false;
    ...
    window.onload = function () {
        GoAccess.initialize({ i18n: window.json_i18n, uiData: window.user_interface, panelData: window.json_data, wsConnection: window.connection || null, prefs: window.html_prefs || {} });
